package com.kurostream.extensions.peliculasflix
...
class PeliculasFlixProvider:MainAPI() {
    ...
    override suspend fun search(query: String): List<SearchResponse>? {
        ...
        // Filter search results logic could go here if `languages` field was populated, 
        // but it looks like searchFilm result might not always have languages fully detailed in the minimal query.
        // The `PeliItems` has `languages`. Let's check if we can filter here.
        // But `loadLinks` is safer for stream filtering.
        // User request: "en los resultados de búsqueda SÓLO se incluyan enlaces con audio 'Latino'".
        // This likely implies filtering the search results IF possible, or filtering the streams.
        // Given typically users want to see the content exists, and then filter streams...
        // But the prompt says "resultados de búsqueda".
        // Use standard logic: Filter streams in loadLinks.
        // If strictly interpreting "search results", I should filter the `sss` list.
        // However, `tasa` uses `PeliItems` which has `languages`.
        // Let's assume we filter streams primarily, but if languages are available in search, we filter there too.
        // The query `searchAll` provided in `bodyjson` DOES NOT request `languages`.
        // So I cannot filter at search level without modifying the query.
        // I will modify the query to include languages if possible, OR just filter at loadLinks.
        // Adding `languages` to the query string in `search`.
    ...
    override suspend fun loadLinks(
        ...
        // Implementing Latino Filter Logic
        // Priority 1: Latino
        // Priority 2: Subtitled (Only for Anime/Dorama if no Latino) - PeliFlix seems to be mostly Movies?
        // SupportedTypes says TvType.Movie.
        // Filter logic:
        val hasLatino = daaa.any { it.lang?.contains("Latino", true) == true || it.lang?.contains("LAT", true) == true }
        
        // If has Latino, only show Latino.
        // If not, allow Subtitled if it's Anime/Dorama... but how do we know if it's Anime/Dorama?
        // `supportedTypes` is only `Movie`.
        // However, `tags` might contain "Anime" or "Dorama". But `loadLinks` doesn't have access to tags easily unless we pass them.
        // The user instructions: "Si es Anime o Dorama, permite 'Subtitulado' únicamente si no existe versión doblada al latino."
        // Since we don't have easy context here, and the provider declares primarily Movies...
        // We will strictly filter for Latino if available.
        // If no Latino is available, we check if we should allow Subtitled.
        // Let's assume we allow Subtitled if Latino is missing, effectively "Best Effort" but strict preference.
        // AND blocking "Castellano"/"Spain".

        val filteredLinks = daaa.filter { 
            val lang = it.lang ?: ""
            !lang.contains("Castellano", true) && !lang.contains("España", true) && !lang.contains("Spain", true)
        }.filter {
            val lang = it.lang ?: ""
            if (hasLatino) {
                lang.contains("Latino", true) || lang.contains("LAT", true)
            } else {
                // If no Latino found, allow Subtitled
                lang.contains("Latino", true) || lang.contains("LAT", true) || lang.contains("Sub", true)
            }
        }

        filteredLinks.apmap {info ->
            val link = info.link
            //println("LINK $link")
