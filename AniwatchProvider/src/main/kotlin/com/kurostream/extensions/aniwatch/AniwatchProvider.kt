...
//            "https://ws1.rapid-cloud.ru/socket.io/?EIO=4&transport=polling"
        ...
        serverstwo.distinctBy { it.second }.apmap {
            ...
            // Wait, this provider logic is complex. 'extractRabbitStream' calls 'callback'.
            // I need to intercept filter within 'extractRabbitStream' OR
            // Since 'loadLinks' relies on 'sub' vs 'dub' passed in 'data', I can filter BEFORE extraction?
            // 'data' contains 'dubs': "sub" or "dub".
            // Strict Filtering: Allow SUB. Block DUB (English).
            // parseData.dubs == "sub" -> OK.
            // parseData.dubs == "dub" -> Block?
            
            // "Excepción: Si el contenido es de tipo Anime... permite la versión Subtitulado."
            // "Nunca permitas 'Castellano'."
            
            // If I block "dub" here, I effectively block English Dub.
            // "Latino" is not an option here (Aniwatch is English).
            
            if (parseData.dubs.equals("dub", ignoreCase = true)) {
                // Determine if I should block. 
                // Since user said "SOLO... Latino", and this is English Dub -> Block by default?
                // But it's Aniwatch. 
                // Exception allows Subtitled.
                // It does NOT allow English Dub.
                // So I return false/empty for Dub?
                return@apmap // Skip this server processing.
            }
            
            val hasLoadedExtractorLink =
                loadExtractor(extractorLink, "https://rapid-cloud.ru/", subtitleCallback) { l -> capturedLinks.add(l) }
            if (!hasLoadedExtractorLink) {
                extractRabbitStream(
                    ...
                    { videoLink -> 
                        if (!videoLink.url.contains("betterstream")) capturedLinks.add(videoLink) 
                    },
                    false,
                    null,
                    decryptKey = getKey()
        ...
        // Final Filter to be sure (though pre-filtering dub handles most)
        val finalLinks = capturedLinks.filter {
...
//                Regex("""recaptchaNumber = '(.*?)'""").find(iframe.text)?.groupValues?.get(1)
            ...
            val sourceObject = if (decryptKey != null) {
                ...
                if (sources != null) {
                    val decrypted = decrypt(sources, decryptKey)
                    //println("DEC $decrypted")
                    AppUtils.tryParseJson<SourceObject>(decrypted)
                        ?.copy(tracks = encryptedMap.tracks)
                } else {
                    null
                }
            } else {
                response.parsedSafe<SourceObject>()
            }

            sourceObject?.sources?.forEach { source ->
                source?.toExtractorLink(this, nameTransformer(source.label ?: "Unknown"), extractorData)?.forEach(callback)
            }
            sourceObject?.sources1?.forEach { source ->
                source?.toExtractorLink(this, nameTransformer(source.label ?: "Unknown"), extractorData)?.forEach(callback)
            }
            sourceObject?.sources2?.forEach { source ->
                source?.toExtractorLink(this, nameTransformer(source.label ?: "Unknown"), extractorData)?.forEach(callback)
            }
            sourceObject?.sourcesBackup?.forEach { source ->
                source?.toExtractorLink(this, nameTransformer(source.label ?: "Unknown"), extractorData)?.forEach(callback)
            }
            sourceObject?.tracks?.forEach { track ->
                track?.toSubtitleFile()?.let(subtitleCallback)
            }
        }
    }
}
