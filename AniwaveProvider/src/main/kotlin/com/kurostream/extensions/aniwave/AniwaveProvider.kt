...
class AniwaveProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        // Strict Filtering Priority: Latino > Subbed (fallback)
        // Aniwave is English provider, so no "Latino" here usually.
        // Exception: Allow Subtitled. Blacklist Dub?
        // If the user wants ONLY Latino, but Aniwave has no Latino, exception allows Subbed.
        
        if (parseData.type.equals("dub", ignoreCase = true)) {
             // Aniwave Dub is English. Rule says exception for Subbed only if no Latino.
             // Usually English Dub is not what they want if they want Latino.
             return false
        }
        ...
        aas.apmap { (sName, sId) ->
            ...
            if (vids || mclo) {
                ...
                if (encUrl != null) {
                    ...
                    val vizId = comps[comps.lastIndex];
            ...
            if (!sName.isNullOrEmpty() && !vids || !mclo) {
                ...
                loadExtractor(bbbs, subtitleCallback) { l -> capturedLinks.add(l) }
            }
        }
        
        // Final filter to ensure no Castellano/Spain
