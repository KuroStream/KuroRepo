package com.kurostream.extensions.bflix
...
data class TestingSubsItem(
    ...
    override suspend fun loadLinks(
        ...
        val servers = document.select(".film-server").mapNotNull {
            val serverId = it.attr("data-id")
            val datalinkId = it.attr("data-link-id")
            val newSname = serverName(serverId)
            val serverText = it.text().trim()
            
            // Block Castellano/Spain
            if (serverText.contains("Castellano", true) || 
                serverText.contains("España", true) || 
                serverText.contains("Spain", true)) return@mapNotNull null
                
            Triple(newSname, datalinkId, serverText)
        }.sortedByDescending { (_, _, text) ->
            // Priority 1: Latino/LAT
            if (text.contains("Latino", true) || text.contains("LAT", true)) 2
            // Priority 2: Subtitled (implicit Spanish context from request)
            else if (text.contains("Sub", true)) 1
            else 0
        }

        servers.apmap {(sName, sId, _) ->
            ...
            subsUrl.filter { 
                val label = it.label ?: ""
                !label.contains("Castellano", true) && !label.contains("España", true) && !label.contains("Spain", true)
            }.apmap {
