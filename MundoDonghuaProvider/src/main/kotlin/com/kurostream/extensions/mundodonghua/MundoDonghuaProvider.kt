package com.kurostream.extensions.mundodonghua
...
class MundoDonghuaProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request : MainPageRequest): HomePageResponse {
        ...
        val latestEps = app.get(mainUrl, timeout = 120).document.select("div.row .col-xs-4").mapNotNull {
            ...
            val url = it.selectFirst("a")?.attr("href")?.replace(epRegex,"")?.replace("/ver/","/donghua/")
                ?.replace(Regex("$/|$(\\d+)"),"") ?: return@mapNotNull null
        ...
        items.add(HomePageList("Últimos episodios", latestEps))

        urls.forEach { (url, listName) ->
            val home = app.get(url, timeout = 120).document.select(".col-xs-4").mapNotNull {
                val title = it.selectFirst(".fs-14")?.text() ?: return@mapNotNull null
                val poster = it.selectFirst(".fit-1 img")?.attr("src") ?: ""
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        return app.get("$mainUrl/busquedas/$query", timeout = 120).document.select(".col-xs-4").mapNotNull {
            val title = it.selectFirst(".fs-14")?.text() ?: return@mapNotNull null
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        doc.select("ul.donghua-list a").forEach {
        ...
        secondDoc.select("div.sm-row.bg-white.pt-10.pr-20.pb-15.pl-20 div.row div.item.col-lg-2.col-md-2.col-xs-4").forEach {
        ...
        return newAnimeLoadResponse(title, url, tvType) {
            posterUrl = poster
            addEpisodes(DubStatus.Subbed, newEpisodes.sortedBy { it.episode }.distinctBy { it.episode })
            showStatus = status
            plot = description
            tags = genres
        }
    ...
    override suspend fun loadLinks(
        ...
        app.get(data).document.select("script").forEach { script ->
            if (script.data().contains("eval(function(p,a,c,k,e")) {
                ...
                packedRegex.findAll(script.data()).forEach {
                    val unpack = getAndUnpack(it.value).replace("diasfem","embedsito")
                    fetchUrls(unpack).forEach { url ->
                        val newUrl = url.replace("https://sbbrisk.com","https://watchsb.com")
                        loadExtractor(newUrl, data, subtitleCallback) { l -> capturedLinks.add(l) }
                    }
                    if (unpack.contains("protea_tab")) {
                        ...
                        if (!ssee.isNullOrEmpty()) {
                            ...
                            if (!vidID.isNullOrEmpty()) {
                                ...
                                loadExtractor(newLink, subtitleCallback) { l -> capturedLinks.add(l) }
                            }
                        }
                    }
                    if (unpack.contains("asura_player")) {
                        ...
                        if (!aass.isNullOrEmpty()) {
                            ...
                            if (test.contains(Regex("#EXTM3U"))) {
                                generateM3u8("Asura", aass, "").forEach { l -> capturedLinks.add(l) }
                            }
                        }
                    }
                }
