package com.kurostream.extensions.animejl
...
class AnimeJlProvider : MainAPI() {
    ...
    override suspend fun loadLinks(
        ...
        val frameUrl = app.get(data).document.select("script")
        ...
        if (frameUrl != null) {
             regex.findAll(frameUrl.html()).map { it.groupValues.get(2) }.toList().apmap {
                loadExtractor(it, data, subtitleCallback) { link ->
        ...
        val allowedLinks = capturedLinks.filter { 
            ...
            !it.name.contains("European Spanish", true) &&
            !it.name.contains(" ES", true) // Careful with ES, might match other things, but usually space ES or [ES] is safe-ish.
        }
        ...
        val finalLinks = if (hasLatino) {
            ...
            // 3. Allow Subtitled if no Latino (AnimeJl is mostly Anime)
            allowedLinks // Allow whatever is left (Subtitled, or Unspecified) as long as it's not blocked.
        }
