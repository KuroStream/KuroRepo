package com.kurostream.extensions.monoschinos
...
class MonoschinosProvider : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request : MainPageRequest): HomePageResponse {
        ...
        val latestEps = app.get(mainUrl, timeout = 120).document.select(".row-cols-xl-4 li article").mapNotNull {
            val title = it.selectFirst("h2")?.text() ?: it.selectFirst("h2.text-truncate")?.text() ?: return@mapNotNull null
        ...
        items.add(HomePageList("Capítulos actualizados", latestEps, isHorizontal))
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        val epList = if (caplist.isNotBlank()) {
